...
func (ct *RobotLogin) RobotLogin(c *gin.Context) {
	var req struct {
		...
		IsPretender *bool  `form:"is_pretender" json:"is_pretender" binding:"required"`
	}
...
<body>
	...
	data, err := service.NewRobotLoginService(c).LoginSliderVerify(req)
	if err != nil {
...
func (ct *RobotLogin) LoginSliderVerifySubmit(c *gin.Context) {
	...
	err := service.NewRobotLoginService(c).LoginSliderVerifySubmit(req)
	if err != nil {
		log.Println("LoginSliderVerifySubmit ->", err.Error())
		resp.ToErrorResponse(err)
		return
