...
type Robot struct {
	...
	DockerImage   string         `gorm:"column:docker_image;type:varchar(512);not null;default:'registry.cn-shenzhen.aliyuncs.com/houhou/wechat-ipad:latest'" json:"docker_image"` // 当前机器人的Docker镜像
...
func (r *Robot) GetProxy() *Proxy {
	if len(r.Proxy) == 0 {
		return nil
	}
	var p Proxy
	if err := json.Unmarshal(r.Proxy, &p); err != nil {
		return nil
	}
	return &p
}
