...
CREATE TABLE IF NOT EXISTS `chat_room_settings` (
  ...
  -- 微信红包通知
  `wxhb_notify_enabled` BOOLEAN DEFAULT FALSE COMMENT '是否启用微信红包通知功能',
...
CREATE TABLE IF NOT EXISTS `chat_room_members` (
  ...
  `is_blacklisted` BOOLEAN DEFAULT FALSE COMMENT '是否在黑名单',
...
CREATE TABLE IF NOT EXISTS `system_settings` (
  ...
  `notification_type` ENUM('push_plus', 'email', 'wechat_work_app') NOT NULL DEFAULT 'push_plus' COMMENT '通知方式：push_plus-推送加，email-邮件，wechat_work_app-企业微信应用',
  `push_plus_url` VARCHAR(255) DEFAULT '' COMMENT 'Push Plus的URL',
  `push_plus_token` VARCHAR(255) DEFAULT '' COMMENT 'Push Plus的Token',
  `wechat_work_corp_id` VARCHAR(255) DEFAULT '' COMMENT '企业微信企业ID',
  `wechat_work_agent_id` VARCHAR(255) DEFAULT '' COMMENT '企业微信应用AgentId',
  `wechat_work_secret` VARCHAR(255) DEFAULT '' COMMENT '企业微信应用Secret',
  `wechat_work_proxy_url` VARCHAR(255) DEFAULT '' COMMENT '企业微信代理地址',
  `wechat_work_to_user` VARCHAR(512) DEFAULT '' COMMENT '企业微信推送用户ID，留空默认ALL',
